...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--results-dir", type=Path, default=DEFAULT_RESULTS_DIR, help="Register results root or one batch folder.")
...
def main(argv=None) -> int:
    ...
    import_tag = cloud_import.safe_tag(
        args.import_tag or "register_fallback_{0}".format(datetime.now().strftime("%Y%m%d_%H%M%S"))
    )
    return cloud_import.import_bundle(make_import_args(args, Path(summary["output_path"]), import_tag))
